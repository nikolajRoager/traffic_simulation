...
/*All tests related to LOADING are in ONE FILE
Because I re-use some of the same mock classes in several tests
*/
...
TEST(Test_Loading, Load_CityNetwork_From_Stream)
{
    ...
    }
    {
        ...
        std::stringstream S(JSon_City_String);
        CityNetwork City(S);
        ASSERT_EQ(City.getNodesSize(),2);
        ASSERT_EQ(City.getRoadsSize(),1);

        //Check that this simple city network handles out of bounds
        ASSERT_THROW(City.getRoad(City.getRoadsSize()),road_address_exception);
        ASSERT_THROW(City.getNode(City.getNodesSize()),node_address_exception);

        std::shared_ptr<const Road> R =City.getRoad(0);
        ASSERT_EQ(R->getRoadID(),0);
        ASSERT_EQ(R->getType(),motortrafficroad);
        ASSERT_EQ(R->getLanes(),3);
        ASSERT_EQ(R->getNoOvertake(),true);
        ASSERT_EQ(R->getOneWay(),false);
